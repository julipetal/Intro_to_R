...
vec = 				# a numeric vector, might have to use as.vector(), as.numeric(), na.omit()
numBins =                     	# number of histogram’s subintervals/bins
fileName =

# MIN and MAX will always be integers
MIN = floor(min(vec))     # MIN = round(min(vec)-0.5, digits=0)
MAX = ceiling(max(vec))   # MAX = round(max(vec)+0.5, digits=0) 
